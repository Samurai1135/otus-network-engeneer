...
`Цель:`
Настроить DHCPv4 (Lab-03-1.unl) & Настроить DHCPv6 (Lab-03-2.unl)
...
- `Часть 5.` Настройка DHCPv6-сервера на роутере R1. 
...
`network {network}`                            //Указываем подсеть, из которой будет раздача адресов IPv4 <br>
`default-router {address}`                     //Шлюз по-умолчанию (интерфейс, смотрящий в подсеть) <br>
...
ip route 0.0.0.0 0.0.0.0 10.0.0.2
~~~
`10.0.0.2` // адрес интерфеса на R2, смотрящего в сторону R1
- ## Часть 3. Настройка DHCP-реле на роутере R2
