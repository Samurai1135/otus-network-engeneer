...
На транковом соединении между SW4(e0/0) и SW5(e0/1) порт SW4(e0/0) является Alternate-портом, т.к.в случае отсутствия транковых соединений SW5-SW1 и SW5-SW2 его стоимость Cost будет равна 300. Но транк SW5-SW1 - прямое подключение SW5 к Root Bridge и стоимость порта e0/2 Cost = 100. При наличии портов с наименьшей стоимостью порты с большей стоимостью становятся ненужными, и протокол STP переводит их в режим Alternate (нет передачи данных).

В случае порта SW4(e0/1) - порт переходит в Alternate, т.к. маршрут через SW2 является более приоритетным, чем через SW3, т.к. BridgeID Address SW2 меньше, чем у SW3, хотя стоимость магистральных портов одинаковая (Cost = 200)
...
<br>
Как видно из результатов команды "show spanning-tree active" на коммутаторе Switch5(SW5) интерфейс eth1/1 перешел в режим Alternate-port, т.к. PortID отправителя SW1(e0/2) больше, чем у SW1(eth1/1), хотя оба транка связывают коммутатор с RootBridge и имеют одинаковую стоимость Cost.
