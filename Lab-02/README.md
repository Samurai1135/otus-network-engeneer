...
В качестве корневого моста (Root Bridge) был добавлен коммутатор Switch-1, т.к. при одинаковых значениях (в лабораторной работе) BridgeID приоритет выбора падает на коммутатор, чей BridgeID-адрес меньше остальных.
...
На транковом соединении между SW4(e0/0) и SW5(e0/1) порт SW(e0/0) является Alternate-портом, т.к.в случае отсутствия транковых соединений SW5-SW1 и SW5-Sw2 его стоимость Cost будет равна 300. Но транк SW5-SW1 - прямое подключение SW5 к Root Bridge и стоимость порта e0/2 Cost = 100. При наличии портов с наименьшей стоимостью порты с большей стоимостью становятся ненужными, и протокол STP переводит их в режим Alternate (нет передачи данных).
