...
Стоимость пути Root Path Cost на портах коммутаторов равна 100, что говорит о прямом подключении к RouteBridge. Исключение составляет коммутатор Switch4(SW4), чья стоимость пути Root Path Cost = 200, т.к. на маршрутах между ним и RootBridge есть еще по одному сетевому устройству - коммутатор Switch2(SW2) (по пути слева на схеме) и Switch3(SW3) (по пути справа на схеме)

#### Логика выбора Alternate-портов (неактивных портов):
На транковом соединении между SW4(e0/0) и SW5(e0/1) порт SW4(e0/0) является Alternate-портом, т.к. в случае отсутствия транковых соединений SW5-SW1 и SW5-SW2  стоимость его пути RootPathCost будет равна 300. Но транк SW5-SW1 - прямое подключение SW5 к Root Bridge и стоимость пути до порта e0/2 Cost = 100. При наличии портов с наименьшей стоимостью пути порты с большей стоимостью пути становятся ненужными, и протокол STP переводит их в режим Alternate (нет передачи данных).
